      ...
      {currentPage === 'shop' ? (
        ...
        >
          ...
          <nav className="fixed top-0 left-0 w-full z-50 px-6 py-4 flex justify-between items-center backdrop-blur-md bg-white/30 border-b border-white/10">
            <div className="text-lg tracking-widest text-slate-800 font-bold cursor-pointer" onClick={handleGoHome}>
              BAGUA VIBES
            </div>
          ...
          <div className={`fixed inset-0 z-40 bg-white/95 backdrop-blur-2xl flex flex-col items-center justify-center transition-all duration-500 ${isMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-full pointer-events-none'}`}>
            <div className="text-center space-y-8 max-w-lg px-6">
              ...
              <div className="mb-8">
                <button onClick={() => navigateToShop('all')} className="text-xl text-slate-700 hover:text-amber-700 transition-colors tracking-widest font-medium">Shop All Products</button>
              </div>
              <div className="flex flex-col gap-y-4 md:gap-y-6 w-full max-h-[60vh] overflow-y-auto px-4 custom-scrollbar">
                {menuItems.map((item) => (
                  <a key={item} href={`#${item.toLowerCase().replace(/\s+/g, '-')}`} onClick={(e) => { e.preventDefault(); navigateToShop(item); }} className="text-lg sm:text-xl md:text-2xl text-slate-700 hover:text-amber-700 transition-colors tracking-[0.2em] font-medium py-2 border-b border-amber-700/5 last:border-0 block w-full">
                    {item}
                  </a>
                ))}
              </div>
          ...
          <section className="min-h-screen flex flex-col items-center justify-start pt-32 px-6 relative z-10">
            ...
            <h1 className={`text-5xl md:text-8xl text-center mb-6 transition-all duration-1000 ${visibleElements.has('title') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`} style={{ filter: 'url(#liquid-filter)', letterSpacing: '0.1em', textShadow: '0 0 8px rgba(180, 83, 9, 0.1)' }}>BAGUA VIBES</h1>
            ...
            <div className={`mb-24 transition-all duration-1000 ${visibleElements.has('cta') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
              <div className="flex gap-4">
                <a href="#categories" className="relative px-8 py-3 bg-transparent text-amber-700 border border-amber-700 rounded-full text-[11px] uppercase tracking-widest font-bold transition-all duration-500 hover:bg-amber-700 hover:text-white hover:shadow-lg hover:shadow-amber-700/20 inline-flex items-center">
                  Enter The Sanctuary <span className="inline-block ml-3 transform group-hover:translate-y-1 transition-transform">↓</span>
                </a>
                <button onClick={() => navigateToShop('all')} className="relative px-8 py-3 bg-amber-700 text-white border border-amber-700 rounded-full text-[11px] uppercase tracking-widest font-bold transition-all duration-500 hover:bg-amber-800 hover:shadow-lg hover:shadow-amber-700/20 inline-flex items-center">
                  Shop Now <span className="inline-block ml-3">🛒</span>
                </button>
              </div>
            </div>

            <div id="categories" className="w-full max-w-6xl py-12 mb-20">
              <div className="text-center mb-16"><h2 className="text-2xl md:text-4xl text-slate-800 tracking-[0.25em] font-medium">Choose Your Harmony</h2><div className="mt-4 h-[1.5px] w-24 bg-amber-700/20 mx-auto"></div></div>
                ...
                {categories.map((cat, i) => (
                  <div key={cat.name} className={`flex flex-col items-center group transition-all duration-700 ${visibleElements.has('categories') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`} style={{ transitionDelay: `${i * 100}ms` }}>
                    <div className="relative w-16 h-16 md:w-36 md:h-36 lg:w-44 lg:h-44 rounded-full bg-white/60 backdrop-blur-md border border-amber-700/15 flex items-center justify-center cursor-pointer overflow-hidden shadow-md shadow-black/5 transition-all duration-700 hover:scale-105 hover:border-amber-700 hover:bg-white/95 mb-4 sm:mb-8">
                      <img src={cat.img} alt={cat.name} onClick={() => navigateToShop(cat.name)} loading="lazy" className="absolute inset-0 w-full h-full object-cover opacity-80 transition-all duration-1000" onError={(e) => { (e.target as HTMLImageElement).src = `https://via.placeholder.com/400x400/f8fafc/b45309?text=${cat.name}`; }} />
                    </div>
                    <h3 className="text-[7px] sm:text-[10px] md:text-xs tracking-[0.1em] sm:tracking-[0.35em] text-slate-500 group-hover:text-amber-700 transition-colors duration-500 uppercase font-bold text-center">{cat.name}</h3>
                  </div>
                ))}
              </div>
            </div>
          ...
          <footer className="bg-white/90 backdrop-blur-3xl border-t border-slate-100 pt-24 pb-16 px-6 relative z-10">
            <div className="max-w-6xl mx-auto">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-16 mb-20">
                <div className="md:col-span-1 text-center md:text-left"><h2 className="text-xl mb-4 font-medium">BAGUA VIBES</h2></div>
                <div className="text-center md:text-left"><h4 className="text-[10px] uppercase tracking-[0.5em] text-amber-700 mb-6 font-bold">Sanctuary</h4><p className="text-sm font-semibold text-stone-700 mb-2 italic">Beirut, Lebanon</p><a href="tel:+9613953615" className="text-sm font-medium text-stone-500 hover:text-amber-700 transition-colors tracking-widest">+961 3 953 615</a></div>
                <div className="text-center md:text-left"><h4 className="text-[10px] uppercase tracking-[0.5em] text-amber-700 mb-6 font-bold">Follow Us</h4><div className="flex justify-center md:justify-start gap-8 text-stone-500 text-sm font-semibold"><a href="#" className="hover:text-amber-700 transition-all">Instagram</a><a href="#" className="hover:text-amber-700 transition-all">Facebook</a></div></div>
                <div className="text-center md:text-left"><h4 className="text-[10px] uppercase tracking-[0.5em] text-amber-700 mb-6 font-bold">Secure Payments</h4><div className="flex flex-wrap justify-center md:justify-start gap-4 opacity-50 grayscale hover:grayscale-0 transition-all"><div className="px-3 py-1 border border-stone-200 text-[9px] font-bold rounded bg-stone-50">VISA</div><div className="px-3 py-1 border border-stone-200 text-[9px] font-bold rounded bg-stone-50">APPLE PAY</div></div></div>
              </div>
              <div className="pt-12 border-t border-stone-100 text-center flex flex-col gap-2">
                <p className="text-[10px] text-stone-300 tracking-[0.7em] uppercase font-bold">&copy; 2026 BAGUA VIBES • THE SPIRIT OF THE HORSE</p>
                <p className="text-[9px] text-stone-400 tracking-[0.3em] uppercase">Powered by <a href="https://theahmadcodes.com" target="_blank" rel="noopener noreferrer" className="text-amber-700 hover:text-amber-900 font-bold transition-colors">theahmadcodes</a></p>
              </div>
            </div>
          </footer>
